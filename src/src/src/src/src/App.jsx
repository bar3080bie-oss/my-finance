      ...
      try {
        ...
        rows.forEach((row, i) => {
          ...
          const smartCat2 = (desc, ענף) => {
            ...
            if (/שופרסל|רמי לוי|ויקטורי|מגא|יינות ביתן|סופר|מגנום/.test(d)) return "סופרמרקט";
      if (/מכולת|מינימרקט|מכולת גורנר|סופר אברמל|אלונית/.test(d)) return "מכולת";
      if (/קיוסק|קיוסק בסר/.test(d)) return "קיוסק";
      if (/קצב|קצביה|בשרי|בשר/.test(d)) return "קצביה";
      if (/ירקן|ירקות|פירות|ירק/.test(d)) return "ירקן";
      if (/מאפיי|מאפים|לחם|בייגל|פיתה|מאפה/.test(d)) return "מאפייה";
